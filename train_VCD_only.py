        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                if (
                    ...
                    # ============================================================
                    # VCD：基于 3D 体素重建误差的多视角一致致密化（论文 3.2 节）
                    # ------------------------------------------------------------
                    # 替代之前的 2D 像素归因方案（compute_gaussian_score_r2gs）：
                    #   - 像素归因在 X 射线穿透积分下会被均匀涂抹（实测中位数 400+，
                    #     >1 通过率 99%，分位数过滤无效）
                    #   - 体素归因是局部量：每个高斯只取其 xyz 处的 vol_err 作为分数，
                    #     分布预期呈双峰（误差区高、正确区低），分位数才真的能筛
                    # ============================================================
                    importance_score, _ = compute_gaussian_score_voxel(
                        gaussians,
                        queryfunc,
                        scene.vol_gt,
                        scanner_cfg,
                    )
                    # 用 importance_score 自身的 p70 作为动态阈值
                    nonzero = importance_score[importance_score > 0]
                    if len(nonzero) > 0:
                        importance_thresh_val = torch.quantile(
                            nonzero.float(), 0.70
                        ).item()
                    else:
                        importance_thresh_val = 0.0  # 全零时兜底（早期训练可能发生）

                    # ★ 诊断打印：在 1000/5000/10000/14000 轮观察分布形态
                    #   目的：验证体素归因相对像素归因是否真的恢复了"分辨力"
                    if iteration in (1000, 5000, 10000, 14000):
                        log_score_distribution(
                            iteration, importance_score,
                            importance_thresh_val, name="VCD-VOXEL"
                        )
                    ...
                    gaussians.densify_and_prune(
                        ...
                        pruning_score=None,                       # 切互搏（VCD 不做剪枝）
                        importance_thresh=importance_thresh_val,  # 动态 p70
                    )
